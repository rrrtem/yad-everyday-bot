...
export const MSG_CHANGE_MODE_SUCCESS = (newMode: string) => {
  const threadId = newMode === 'text' ? PUBLIC_REMINDER_THREAD_ID_TEXT : PUBLIC_REMINDER_THREAD_ID_IMAGE;
  const groupChatId = "2366470605"; // TELEGRAM_GROUP_CHAT_ID
  const threadLink = `https://t.me/c/${groupChatId}/${threadId}`;
  
  return `Отлично! Теперь ты делаешь ${newMode === 'text' ? 'тексты' : 'картинки'}.

Теперь присылай свои апдейты в этот тред: ${threadLink}`;
};
