...
// 1. Создайте топик "Тексты" в групповом чате
// 2. Отправьте команду /get в топик
// 3. Обновите значение ниже на актуальный ID
export const PUBLIC_REMINDER_THREAD_ID_TEXT = 2; // Топик для участников режима "тексты"

// Telegram ID администраторов бота
export const ADMIN_TELEGRAM_IDS = [149365895, 219047724]; // Добавьте ID второго админа вместо 123456789

// Для обратной совместимости (deprecated - использовать ADMIN_TELEGRAM_IDS)
export const OWNER_TELEGRAM_ID = ADMIN_TELEGRAM_IDS[0];
...
Здесь мы каждый день делаем небольшое творческое дело. Я буду помогать тебе участвовать в проекте: присылать напоминания о дедлайнах, следить за твоей активностью и пропусками. Если возникнут вопросы или что-то пойдёт не так — пиши @rrrtem.`;
...
export const MSG_LEFT_CHAT_DAYS_SAVED = (daysLeft: number) => `👋 Ты покинул чат челленджа, но твоя подписка всё ещё активна, чтобы её отменить — @tribute.

Если ты отменишь подписку сегодня, у тебя сохранится ${daysLeft} ${pluralizeDays(daysLeft)} подписки! Ты можешь вернуться в любой момент через команду /start и использовать оставшееся время.`;

export const MSG_MODE = `Пока у нас доступно одно направление — мы пишем тексты. Эссе, наблюдения за собой или миром, дневники, посты, анонсы и любые другие жанры. Другие направления будут скоро, следите за обновлениями в <a href="https://www.instagram.com/clarity.and.movement/">инстаграме</a>.`;
...
• Для участников любого сезона проекта <a href="https://www.instagram.com/clarity.and.movement/">«Ясность&Движение»</a> — ₽2900 в месяц
• Подписка автоматически продлевается каждый месяц, её можно отменить в любой момент
• При отмене подписки и выхода из чата до её конца, неиспользованные дни сохраняются на будущее
`;

export const MSG_PROMO = `Если у тебя есть промокод — введи его, чтобы получить специальную цену.`;
...
Сразу переходи в чат участников:
${CHALLENGE_JOIN_LINK}
`;
...
export const MSG_CONTINUE_PACE_SELECTION = (mode: string) => `⏰ Твой режим "Тексты".
...
`ℹ️ Ты уже участвуешь в режиме **Тексты**.`;
