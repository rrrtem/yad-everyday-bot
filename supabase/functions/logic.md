...
**Реализация:** handleComebackCommand в commandHandler.ts
...
2. **Отправка выбора** — показывает только доступные для переключения режимы (исключая текущий):
   - Если текущий режим "text" — показывает только 🎨 "Картинки" → callback_data: "change_mode:image"  
   - Если текущий режим "image" — показывает только 📝 "Тексты" → callback_data: "change_mode:text"
   - Если все режимы настроены — показывает MSG_CHANGE_MODE_ALL_SET без кнопок
...
- CALLBACK_CHANGE_MODE_TEXT, CALLBACK_CHANGE_MODE_IMAGE — callback data для кнопок

**Эталонные переменные:**
- mode: "text" | "image" (из AVAILABLE_MODES)
- PUBLIC_REMINDER_THREAD_ID_TEXT, PUBLIC_REMINDER_THREAD_ID_IMAGE — ID топиков для напоминаний
...
3. Разделяет по режимам: mode = "text" и mode = "image"
4. Отправляет в соответствующие треды:
   - PUBLIC_REMINDER_THREAD_ID_TEXT для текстовиков
   - PUBLIC_REMINDER_THREAD_ID_IMAGE для картинщиков
...
### Команды системы слотов (только владелец):
- `/open[число]` — Установить количество мест (например, /open20)
- `/close_slots` — Закрыть все места (waitlist режим)
...
- **PUBLIC_REMINDER_THREAD_ID_IMAGE = 855** — ID топика для картинщиков
- **OWNER_TELEGRAM_ID = 149365895** — ID владельца бота для отчётов
...
**Режимы и ритмы:**
- **AVAILABLE_MODES** — { TEXT: "text", IMAGE: "image" }
